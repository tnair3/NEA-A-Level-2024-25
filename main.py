...
#CODE =================================================================================================================================================================================================
pygame.display.set_caption(constants.eastereggnames[randint(0, 10)])
...
def playgame():
    ...
    database.addnewscores(user, date, time, score, cleared)
    rulesmenu._back()
    playgamemenu._back()
...
def openleaderboard():
    global scorestable
    sortfilter = sortbutton.get_value()
    filterval = filterinput.get_value()
    scorestable = leaderboardmenu.add.table('Leaderboard Table')
    scorestable.default_cell_padding = 10
    scorestable.add_row(("GameID", "Username", "Date", "Time", "Score", "Cleared"))
    filtermenu._open(leaderboardmenu)
    if filterval == "":
        filterval = None
    leaderboard = database.selectfromscores(filterval)
    leaderboard = list(leaderboard)
    if sortfilter:
        leaderboard = constants.quicksort(leaderboard, 0, len(leaderboard) - 1, 4)
...
def signin():
    ...
    if not accountexists:
        messagebox.showerror("Invalid Login", "Account does not exist")
    elif accountexists:
        if validpass:
            ...
            messagebox.showerror("Invalid Login", "Password incorrect")
...
def createaccount():
    ...
    inputusername = usernamebox.get_value()
    inputpassword = passwordbox.get_value()
    accountadded, usernameerror, passworderror = database.addnewlogindetails(inputusername, inputpassword)
    if not accountadded:
        if usernameerror:
            messagebox.showerror("Invalid Username", "Username already used")
        if passworderror:
...
def changeusername(newusername):
    ...
    success, usernameerror, passworderror = database.updatelogindetails(database.getuserID(username), newusername, None)
    if success:
...
def changepassword(newpassword):
    success, usernameerror, passworderror = database.updatelogindetails(database.getuserID(username), None, newpassword)
    if success:
...
rulestheme = pygame_menu.themes.THEME_BLUE.copy()
rulestheme = pygame_menu.Theme(
                widget_font=pygame_menu.font.FONT_MUNRO,
                title_bar_style=pygame_menu.widgets.MENUBAR_STYLE_NONE,
...
if True:
    mainmenu = pygame_menu.Menu("", width, height, 
                            center_content=True,
                            theme=mytheme)
    mainmenu.add.label('PAC', font_size=72)
    ...
    mainmenu.add.button('Credits', opencreditsmenu)
...
if True:
    ...
    playgamemenu.add.label('PAC', font_size=72)
    playgamemenu.add.label('Game Settings', font_size=32)
    playgamemenu.add.label('', font_size=32)
    playgamemenu.add.button('Play Game', openrulesmenu)
...
#RULES MENU
if True:
    rulesmenu = pygame_menu.Menu("", width, height, 
                            center_content=True,
                            theme=rulestheme,
                            mouse_enabled=False)
    rulesmenu.add.image(rules, scale=(0.4, 0.4))
    rulesmenu.add.button('Play', playgame)
...
if True:
    powerupmenu = pygame_menu.Menu("", width, height, 
                            center_content=True,
                            theme=mytheme)
    powerupmenu.add.label('PAC', font_size=72)
...
if True:
    badpowermenu = pygame_menu.Menu("", width, height, 
                            center_content=True, 
                            theme=mytheme,)
    badpowermenu.add.label('PAC', font_size=72)
...
if True:
    settingsmenu = pygame_menu.Menu("", width, height, 
                            center_content=True, 
                            theme=mytheme)
    settingsmenu.add.label('PAC', font_size=72)
...
if True:
    accountmanagernotsignedin = pygame_menu.Menu("", width, height, 
                            center_content=True, 
                            theme=mytheme)
    accountmanagernotsignedin.add.label('PAC', font_size=72)
...
if True:
    accountmanager = pygame_menu.Menu("", width, height, 
                            center_content=True, 
                            theme=mytheme)
    accountmanager.add.label('PAC', font_size=72)
...
if True:
    changeusernamemenu = pygame_menu.Menu("", width, height, 
                            center_content=True, 
                            theme=mytheme)
    changeusernamemenu.add.label('PAC', font_size=72)
...
if True:
    changepasswordmenu = pygame_menu.Menu("", width, height, 
                            center_content=True, 
                            theme=mytheme)
    changepasswordmenu.add.label('PAC', font_size=72)
...
if True:
    spritesheetmenu = pygame_menu.Menu("", width, height, 
                            center_content=True, 
                            theme=mytheme)
    spritesheetmenu.add.label('PAC', font_size=72)
...
if True:
    filtermenu = pygame_menu.Menu("", width, height, 
                            center_content=True, 
                            theme=mytheme)
    filtermenu.add.label('PAC', font_size=72)
    filtermenu.add.label('Filter Leaderboard', font_size=32)
    filtermenu.add.label('', font_size=32)
    filtermenu.add.button('View Leaderboard', openleaderboard)
    sortbutton = pygame_menu.widgets.ToggleSwitch('Sort by scores', default_state=False, state_text=("No", "Yes"))
    filterinput = pygame_menu.widgets.TextInput('Filter: ')
    filtermenu.add.generic_widget(sortbutton, configure_defaults=True)
    filtermenu.add.generic_widget(filterinput, configure_defaults=True)
...
if True:
    leaderboardmenu = pygame_menu.Menu("", width, height, 
                            center_content=True, 
                            theme=mytheme)
    leaderboardmenu.add.label('PAC', font_size=72)
...
#CREDITS
if True:
    credits = pygame_menu.Menu("", width, height, 
                            center_content=True,
                            theme=mytheme)
    credits.add.label('PAC', font_size=72)
    credits.add.label('Credits', font_size=32)
    credits.add.label('', font_size=22)
    credits.add.button('Back', pygame_menu.events.BACK)
    credits.add.label('', font_size=32)

    credits.add.label('MAIN DEVELOPMENT', font_size=36)
    credits.add.label('Me :(', font_size=20)
    credits.add.label('', font_size=12)

    credits.add.label('MASSIVE THANKS TO', font_size=36)
    credits.add.label('------ Programming Guidance ------', font_size=28)
    credits.add.label('Deven', font_size=20)
    credits.add.label('Hezekias', font_size=20)
    credits.add.label('', font_size=12)

    credits.add.label('------ Playtesters ------', font_size=28)
    credits.add.label('Beatrice', font_size=20)
    credits.add.label('Diego', font_size=20)
    credits.add.label('James', font_size=20)
    credits.add.label('Joel', font_size=20)
    credits.add.label('Rohat', font_size=20)
    credits.add.label('Thaib', font_size=20)
    credits.add.label('Tyrone', font_size=20)
    credits.add.label('', font_size=12)

    credits.add.label('------ Powerup Ideas ------', font_size=28)
    credits.add.label('Adeshola', font_size=20)
    credits.add.label('Antonis', font_size=20)
    credits.add.label('Raven', font_size=20)
    credits.add.label('Victor A', font_size=20)
    credits.add.label('Victor B', font_size=20)
    credits.add.label('', font_size=12)

    credits.add.label('------ Name Ideas ------', font_size=28)
    credits.add.label('Beatrice', font_size=20)
    credits.add.label('Danna', font_size=20)
    credits.add.label('Filip', font_size=20)
    credits.add.label('Gisela', font_size=20)
    credits.add.label('Iman', font_size=20)
    credits.add.label('Kae', font_size=20)
    credits.add.label('Khalid', font_size=20)
    credits.add.label('Olu', font_size=20)
    credits.add.label('Om Sai', font_size=20)
    credits.add.label('Temi', font_size=20)
    credits.add.label('Tyrone', font_size=20)
    credits.add.label('Valentine', font_size=20)
    credits.add.label('', font_size=12)

    credits.add.label('------ Moral Support ------', font_size=28)
    credits.add.label('Beatrice', font_size=20)
    credits.add.label('Diego', font_size=20)
    credits.add.label('Elita', font_size=20)
    credits.add.label('Joel', font_size=20)
    credits.add.label('Thaib', font_size=20)
    credits.add.label('Tyrone', font_size=20)
    credits.add.label('Victor A', font_size=20)
    ...
    credits.add.label('------ The Computer Science Corner ------', font_size=28)
    credits.add.label('Beatrice', font_size=20)
    credits.add.label('Diego', font_size=20)
    credits.add.label('Jack (Part time)', font_size=20)
    credits.add.label('Thaib', font_size=20)
    credits.add.label('', font_size=12)
